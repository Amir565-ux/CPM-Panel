...
inf "Trying venv install…"
if python3 -m venv "$DIR/venv" 2>/dev/null && [[ -f "$DIR/venv/bin/pip" ]]; then
  if "$DIR/venv/bin/pip" install --upgrade pip -q && \
     "$DIR/venv/bin/pip" install $PKGS -q; then
    ok "Packages installed into venv"
    INSTALLED=true
  fi
fi

# Method 2: pip3 --break-system-packages (Ubuntu 22.04+ / Debian 12+)
if [[ "$INSTALLED" == false ]]; then
  rm -rf "$DIR/venv"
  inf "Trying pip3 --break-system-packages…"
  if pip3 install --break-system-packages $PKGS -q 2>/dev/null; then
    ok "Packages installed system-wide (--break-system-packages)"
    INSTALLED=true
  fi
fi

# Method 3: pip3 without flag (older systems)
if [[ "$INSTALLED" == false ]]; then
  inf "Trying pip3 install…"
  if pip3 install $PKGS -q 2>/dev/null; then
    ok "Packages installed system-wide"
    INSTALLED=true
  fi
fi

# Method 4: python3 -m pip (last resort)
if [[ "$INSTALLED" == false ]]; then
  inf "Trying python3 -m pip…"
  if python3 -m pip install --break-system-packages $PKGS -q 2>/dev/null || \
     python3 -m pip install $PKGS -q 2>/dev/null; then
    ok "Packages installed via python3 -m pip"
    INSTALLED=true
  fi
fi
...
if python3 -c "import flask" 2>/dev/null || \
   [[ -f "$DIR/venv/bin/python3" ]] && "$DIR/venv/bin/python3" -c "import flask" 2>/dev/null; then
  ok "Flask verified — ready to run"
else
  wrn "Flask may not be installed correctly. Panel will still attempt to start."
fi
