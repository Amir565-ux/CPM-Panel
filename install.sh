...
def stop():
    """Force-off via destroy (guaranteed). Use /api/vps/shutdown for graceful."""
    err = _require_kvm()
    if err: return err
    ...
    if not VM_RE.match(name):
        return jsonify({"error": "Invalid VM name"}), 400
    r = run_virsh("destroy", name)
    if r["success"]:
        return jsonify({"success": True, "message": f"VPS '{name}' force-stopped"})
    return jsonify({"error": r["error"]}), 400
...
def vps_tmate():
    """Start a tmate session and return the SSH / web connection strings."""
    if not shutil.which("tmate"):
        return jsonify({"error": "tmate is not installed on this server"}), 503
    data    = request.get_json() or {}
    name    = data.get("name", "panel")
    sock_path = f"/tmp/tmate-cpm-{re.sub(r'[^a-z0-9]', '', name.lower())}.sock"
    # Kill any existing session for this name
    ...
    # Wait for tmate to connect and generate tokens
    ssh_cmd = web_url = ""
    for _ in range(20):
        time.sleep(0.5)
        rs = subprocess.run(
    ...
    if not ssh_cmd or ssh_cmd.startswith("#{"):
        return jsonify({"error": "tmate connected but token not ready — try again in a few seconds"}), 500
    log.info(f"tmate session started for '{name}': {ssh_cmd}")
    return jsonify({"success": True, "ssh": ssh_cmd, "web": web_url, "name": name})
...
async function openSsh(name) {
  ...
  document.getElementById('ssh-body').innerHTML = '<div class="ssh-loading">⏳ Starting tmate session for <strong>' + name + '</strong>…<br><small style="opacity:.6">This may take a few seconds</small></div>';
  ...
  try {
    ...
    if (d.success) {
      document.getElementById('ssh-body').innerHTML = `
        <div class="ssh-label">SSH Command</div>
        <div class="ssh-box">${d.ssh}<button class="copy-btn" onclick="copyText('${d.ssh}',this)">Copy</button></div>
        ${d.web ? `<div class="ssh-label">Web Browser</div><div class="ssh-box" style="margin-bottom:0"><a href="${d.web}" target="_blank" style="color:#60a5fa;text-decoration:none">${d.web}</a><button class="copy-btn" onclick="copyText('${d.web}',this)">Copy</button></div>` : ''}
        <div class="ssh-note" style="margin-top:16px">
          ℹ️ Paste the SSH command into your terminal to connect.<br>
          The session is shared — anyone with the link can connect.<br>
          Close this panel or kill the tmate process to end the session.
        </div>`;
    } else {
      document.getElementById('ssh-body').innerHTML = `<div class="ssh-loading" style="color:#dc2626">❌ ${d.error}</div>`;
    }
  } catch(e) {
    document.getElementById('ssh-body').innerHTML = '<div class="ssh-loading" style="color:#dc2626">❌ Request failed — is the panel running?</div>';
  }
}
