...
if python3 -m venv "$DIR/venv" 2>/dev/null && [[ -f "$DIR/venv/bin/pip" ]]; then
  ...
  ok "Python packages installed into $DIR/venv"
else
  wrn "venv creation failed — falling back to system-wide install"
  rm -rf "$DIR/venv"
  # --break-system-packages is required on Ubuntu 22.04+ / Debian 12+
  pip3 install --break-system-packages -r "$DIR/requirements.txt" || \
    pip3 install -r "$DIR/requirements.txt"
  ok "Python packages installed system-wide"
fi
