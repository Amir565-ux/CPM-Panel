...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .act-logo{width:60px;height:60px;background:linear-gradient(135deg,#2563eb,#3b82f6);border-radius:16px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:24px;margin:0 auto 20px}
...
const ACT_KEY       = 'CPM-C4X1-P9B2-G2M1';
...
function activationInit() {
  ...
  if (saved === 'paid') {
    ...
    document.getElementById('act-key-input').addEventListener('input', function() {
    ...
    document.getElementById('act-key-input').addEventListener('keydown', function(e) {
      if (e.key === 'Enter') activatePanel();
    });
  }
}
...
function activatePanel() {
  ...
  if (!key) {
    ...
    err.textContent = 'Please enter your activation key.';
    return;
  }
  if (key !== ACT_KEY) {
    inp.classList.add('error');
    inp.classList.remove('success');
    err.textContent = '❌ Invalid activation key. Check and try again.';
    inp.focus();
    return;
  }
  ...
  err.textContent = '';
  document.getElementById('act-btn-paid').textContent = '✅ Activated!';
  document.getElementById('act-btn-paid').style.background = 'linear-gradient(135deg,#16a34a,#15803d)';
  localStorage.setItem(ACT_STORE_KEY, 'paid');
  setTimeout(() => hideActOverlay('paid'), 700);
}
...
function hideActOverlay(mode) {
  ...
  if (mode === 'paid') {
    ...
    banner.textContent = '✅ Paid Version — Full Access';
  } else {
    banner.className = 'act-mode-banner free';
    banner.textContent = '⚠️ Free Version — Upgrade to unlock premium features';
  }
